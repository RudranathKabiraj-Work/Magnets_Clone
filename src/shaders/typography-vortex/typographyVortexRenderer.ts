...
export function createTypographyVortexRenderer(
  ...
  const buildRings = () => {
    ...
    strays = [];
    for (let strayIndex = 0; strayIndex < 20; strayIndex += 1) {
      strays.push({
  ...
  const spawnPointerDust = (time: number, radius: number) => {
    const options = getOptions();
    if (reduced || time < suction.until || !pointer.inside || dissolve < 0.08 || time - lastSpawn < 64) return;
    lastSpawn = time;
    ...
    const sampleStep = Math.max(6, Math.round(7 * pixelRatio));
    const spawnLimit = Math.max(1, Math.round(20 * options.particleAmount));
    let spawned = 0;
    ...
    const maximum = Math.max(1, Math.round(250 * options.particleAmount));
  ...
  const spawnAmbientDust = (time: number, ambientWidth: number) => {
    const options = getOptions();
    if (reduced || time < suction.until || time - lastAmbientSpawn < 180) return;
    ...
    const spawnLimit = Math.max(1, Math.round(8 * options.particleAmount));
    let spawned = 0;
    for (let attempt = 0; attempt < 180 && spawned < spawnLimit; attempt += 1) {
  ...
  const draw = (time: number) => {
    ...
    /* Dissolve ring indicator disabled */
